...
GamedataMappinVariantReference = {
...
function TargetMarker.UnmarkLastByTag(tag)
    if #markers == 0 then
        return
    end
    for i = #markers, 0, -1 do
        if markers[i].tag == tag then
            local marker = markers[i]
            pcall(function() Game.GetMappinSystem():UnregisterMappin(marker.mappinId) end)
            pcall(function() Game.GetMappinSystem():SetMappinActive(marker.mappingId, false) end)
...
---@param tag string Optional tag for categorization
function TargetMarker.Mark(position, variant, tag, markOnMap, visibleThroughWalls)
    ...
    if visibleThroughWalls == nil then
        mappinData.visibleThroughWalls = true
    else
        mappinData.visibleThroughWalls = visibleThroughWalls
    end

    local mappinId = Game.GetMappinSystem():RegisterMappin(mappinData, 
        Vector4:new(
            position.x,
            position.y,
            position.z,
    ...
    print(mappinId)
    if markOnMap then
        pcall(function() Game.GetMappinSystem():SetMappinActive(mappinData.Id, true) end)
    end
    -- Store position with optional tag
...
-- New function to get markers by tag
---@param tag string Optional tag to filter markers
---@return table
function TargetMarker.GetByTag(tag)
    local filteredMarkers = {}

    for i = 1, #markers do
        if not tag or markers[i].tag == tag then
            table.insert(filteredMarkers, markers[i])
        end
    end
...
-- Modify existing functions to use indexed arrays
function TargetMarker.UnmarkAll()
    print("TargetMarker.UnmarkAll")
    
    for i = #markers, 1, -1 do
        local marker = markers[i]
        pcall(function() Game.GetMappinSystem():UnregisterMappin(marker.mappinId) end)
        pcall(function() Game.GetMappinSystem():SetMappinActive(marker.mappinId, false) end)
        table.remove(markers, i)
    end
end
...
---@return gameObject[]
function TargetMarker.GetAll(autoClear)
    ...
    for i = #markers, 1, -1 do
        local marker = markers[i]
        if TargetMarker.IsActive(marker.target) then
