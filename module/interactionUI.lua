...
function ui.update() -- Run every frame to avoid unwanted changes
	if ui.hubShown and ui.selectedIndex then
		local interactions = GetAllBlackboardDefs().UIInteractions
		if not interactions then
			return
		end
		local selectedIndex = GetAllBlackboardDefs().UIInteractions.SelectedIndex
		if not selectedIndex then
			return
		end
		Game.GetBlackboardSystem():Get(interactions):SetInt(selectedIndex, ui.selectedIndex)
	end
